...
static int __convert_status_code(char* status_code)
{
	...
	for (i = HTTP_STATUS_CODE_LEN - 1; i >= 0; i--) {
		converted_digit = g_ascii_digit_value (status_code[i]);
		converted_status_code += converted_digit * pow(10, HTTP_STATUS_CODE_LEN - i - 1);
	}
...
void __parse_response_header(char* buffer, size_t written, gpointer user_data)
{
	...
	char status_code[HTTP_STATUS_CODE_LEN] = {0, };
	char reason_phrase[HTTP_REASON_PHRASE_LEN] = {0, };
	char* start = NULL;

	if (strncmp (buffer, "HTTP/", HTTP_PREFIX_LEN) == 0) {
		if (strncmp (buffer + HTTP_PREFIX_LEN, "1.0", HTTP_VERSION_LEN) == 0)
			response->version = HTTP_VERSION_1_0;
		else if (strncmp (buffer + HTTP_PREFIX_LEN, "1.1", HTTP_VERSION_LEN) == 0)
			response->version = HTTP_VERSION_1_1;

		start = buffer + HTTP_PREFIX_LEN + HTTP_VERSION_LEN + 1;
		strncpy (status_code, start, HTTP_STATUS_CODE_LEN);

		start += HTTP_STATUS_CODE_LEN + 1;
		strncpy (reason_phrase, start, sizeof(start) - 1);

		response->status_code = __convert_status_code(status_code);
		response->status_text = g_strdup(reason_phrase);
	}
}
