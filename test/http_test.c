...
void __transaction_completed_cb(http_transaction_h transaction, void *user_data)
{
	...
	ret = http_response_get_status_code(transaction, &status);

	DBG("Status(%d)\n", status);
	ret = http_transaction_close(transaction);
...
int test_simple_get(void)
{
	...
	ret = http_create_session(&session, HTTP_SESSION_MODE_NORMAL);
	if (ret != 0) {
		ERR("Fail to create session", ret);
		return 0;
	}

	ret = http_open_transaction(session, HTTP_METHOD_GET, &transaction);
	if (ret != 0) {
		ERR("Fail to open transaction", ret);
		return 0;
	}

	ret = http_request_set_uri(transaction, uri);
	if (ret != 0) {
